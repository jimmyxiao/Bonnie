...
/**
 * This class writes the OS/2 table into a TTF file. This currently produces
 * completely wrong numbers, which make the TTF file unusable under Windows.
 * Use a font file converter to clean out these errors.
 * 
...
public class OS2Writer extends FontFormatWriter {
    ...
    String m_vendId = "NONE"; // FMS: Changed to NONE
    ...
    public void write() throws IOException {
        ...
        // FMS: Let's set this to a value that is in scale
        writeInt16(m_xAvgCharWidth = (m_head.getMax().x - m_head.getMin().x) * 2 / 3);
        writeUInt16(m_usWeightClass);
        writeUInt16(m_usWidthClass);
        writeInt16(m_fsType);

        // FMS: Let's set these to values that are in scale
        writeInt16(m_ySubscriptXSize = (m_head.getMax().x - m_head.getMin().x) * 1 / 3);
        writeInt16(m_ySubscriptYSize = (m_head.getMax().y - m_head.getMin().y) * 1 / 3);
        writeInt16(m_ySubscriptXOffset = 0);
        writeInt16(m_ySubscriptYOffset = -(m_head.getMax().y - m_head.getMin().y) * 1 / 3);
        writeInt16(m_ySuperscriptXSize = (m_head.getMax().x - m_head.getMin().x) * 1 / 3);
        writeInt16(m_ySuperscriptYSize = (m_head.getMax().y - m_head.getMin().y) * 1 / 3);
        writeInt16(m_ySuperscriptXOffset = 0);
        writeInt16(m_ySuperscriptYOffset = (m_head.getMax().y - m_head.getMin().y) * 1 / 3);
        writeInt16(m_yStrikeoutSize = (m_head.getMax().y - m_head.getMin().y) / 100);
        writeInt16(m_yStrikeoutPosition = (m_head.getMax().y - m_head.getMin().y) * 1 / 2);
        ...
        writeUInt16(m_usLastCharIndex = 0xFFFF); // FMS: Allow more characters
                                                 // here
        writeUInt16(m_sTypoAscender = (m_head.getMax().y - m_head.getMin().y)); // FMS:
                                                                                // Set
                                                                                // to
                                                                                // scaled
                                                                                // value
        ...
        writeUInt16(m_usWinAscent = m_head.getMax().y); // FMS: Set to yMax
        writeUInt16(m_usWinDescent);
        writeUInt32(m_ulCodePageRange1);
        writeUInt32(m_ulCodePageRange2);
        writeInt16(m_sxHeight = (m_head.getMax().y - m_head.getMin().y)); // FMS:
                                                                          // Set
                                                                          // to
                                                                          // a
                                                                          // value
                                                                          // in
                                                                          // scale
        writeInt16(m_sCapHeight = m_sxHeight);
        writeUInt16(m_usDefaultChar);
        writeUInt16(m_usBreakChar);
        writeUInt16(m_usMaxContext = 2); // FMS: Allow Kerning
