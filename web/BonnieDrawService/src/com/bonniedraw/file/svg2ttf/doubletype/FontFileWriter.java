...
public class FontFileWriter extends FontFormatWriter {
    ...
    private ArrayList<FontFormatWriter> m_tables = new ArrayList<FontFormatWriter>();
    ...
    public FontFileWriter(RandomAccessFile a_file) {
        ...
        m_glyf = new GlyfWriter(m_loca, m_maxp, m_head, m_hdmx);

        m_hhea = new HheaWriter(m_glyf, m_head);
        m_hmtx = new HmtxWriter(m_glyf, m_hhea);
        m_name = new NameWriter();
        m_post = new PostWriter();
        m_kern = new KernWriter(m_cmap);

        // http://www.microsoft.com/typography/otspec/recom.htm
        // head, hhea, maxp, OS/2, hmtx, LTSH, VDMX, hdmx, cmap,
        // fpgm, prep, cvt, loca, glyf, kern, name, post, gasp, PCLT, DSIG
        /*
         * m_tables.add(m_head); m_tables.add(m_hhea); m_tables.add(m_maxp);
         * m_tables.add(m_os2); m_tables.add(m_hmtx); m_tables.add(m_hdmx);
         * m_tables.add(m_cmap); m_tables.add(m_loca); m_tables.add(m_glyf);
         * m_tables.add(m_name); m_tables.add(m_post);
         */
        ...
        m_tables.add(m_hhea);
        m_tables.add(m_maxp);
        m_tables.add(m_os2); // FMS: not required, but needed for Windows
        m_tables.add(m_name);
        m_tables.add(m_cmap);
        m_tables.add(m_loca);

        m_tables.add(m_hmtx);
        //m_tables.add(m_hdmx); // FMS: Not required
        m_tables.add(m_glyf);
        m_tables.add(m_post);        
        m_tables.add(m_kern);
            }

    public void addKern(char c1, char c2, int kern) {
      m_kern.addKerning(c1, c2, kern);  
    }
    
    /** sets meta information for the font */
    public void setNames(String fontName, String designer, String designerUrl) {
        m_name.setNames(fontName, designer, designerUrl);
    }
     ...
     * 
     * @param a_codeRange
     *            position of the bit. For example, JIS will be 17.
     */
